...
def main():
    ...
    # This streaming harness requires vLLM's token IDs and speculative metrics.
    ...
    parser.add_argument(
        "--benchmark-corpus", type=Path, default=Path("tests/data/m6/english_fixed.json")
    )
    ...
    provenance = {
        "engine": args.engine,
        "kernel_options": {
            key: env.get(key) for key in ["BONSAI_VERIFY_KERNEL", "BONSAI_DRAFT_CONTEXT_GRAPH"]
        },
        "source_sha256": {
            str(p): hashlib.sha256(p.read_bytes()).hexdigest()
            for p in [
                *sorted(Path("src/vllm_bonsai2").glob("*.py")),
                Path("tools/m6_compare.py"),
                Path("tools/m6_http_bench.py"),
            ]
        },
